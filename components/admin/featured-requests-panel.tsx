...
import type { FeaturedPlanKey, FeaturedRequestStatusKey } from "@/lib/featured/config";
...
type FeaturedRequestsApiResponse = {
  success: true;
  requests: FeaturedRequestRow[];
};

const tabs: Array<{ key: "all" | FeaturedRequestStatusKey; label: string }> = [
  { key: "all", label: "All" },
  { key: "new", label: "New" },
...
const fetcher = async (url: string): Promise<FeaturedRequestsApiResponse> => {
  const response = await fetch(url);
  const result = await response.json();
  if (!response.ok || !result.success) {
    throw new Error(result.error ?? "Failed to load featured requests");
  }
  return result;
};

function toActionLabel(action: "move_to_in_review" | "approve" | "reject" | "publish") {
...
export function FeaturedRequestsPanel() {
  const [activeTab, setActiveTab] = useState<"all" | FeaturedRequestStatusKey>("new");
  ...
  const { data, error, isLoading, mutate } = useSWR(
    `/api/admin/featured-requests${query}`,
    fetcher,
  );
  ...
  async function patchRequest(
    ...
    setStatusMessage(`Updated: ${toActionLabel(action)}`);
    await mutate();
  }

  function actionsForStatus(status: FeaturedRequestStatusKey) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-white">Featured Requests</h1>
        <p className="mt-1 text-sm text-zinc-400">
          Review founder applications and manually publish approved profiles.
        </p>
      </div>
      ...
      <div className="space-y-3">
        {isLoading ? <p className="text-sm text-zinc-400">Loading requests...</p> : null}
        ...
        {rows.map((row) => (
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-500">
                  {row.workEmail} • {row.plan.toUpperCase()} • ₹{row.priceInr} / ${row.priceUsd}
                </p>
                <p className="mt-1 text-xs text-zinc-500">
            ...
            <p className="mt-3 text-sm text-zinc-300">{row.productSummary}</p>

            <div className="mt-3 grid gap-2 text-xs text-zinc-400 sm:grid-cols-2">
              <p>Website: {row.websiteUrl ?? "-"}</p>
              <p>LinkedIn: {row.linkedinUrl ?? "-"}</p>
              <p>Country: {row.country ?? "-"}</p>
              <p>Industry: {row.industry ?? "-"}</p>
              <p>Stage: {row.stage ?? "-"}</p>
              <p>External ID: {row.externalSubmissionId ?? "-"}</p>
              <p className="sm:col-span-2">
                Published Founder Entry: {row.publishedFounderEntryId ?? "-"}
              </p>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {actionsForStatus(row.status).map((action) => (
                <button
                  key={`${row.id}-${action}`}
                  type="button"
                  onClick={() => patchRequest(row.id, action)}
                ...
                >
                  {savingId === row.id ? "Saving..." : toActionLabel(action)}
                </button>
              ))}
            </div>
          </div>
