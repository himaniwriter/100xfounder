...
type SavedSearchByIdRouteProps = {
  params: { id: string };
};
...
export async function DELETE(
  request: NextRequest,
  { params }: SavedSearchByIdRouteProps,
) {
  ...
  if (!id) {
    return NextResponse.json(
      { success: false, error: "Invalid saved search id." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const deleted = await prisma.userSavedSearch.deleteMany({
      where: {
        id,
    ...
    return NextResponse.json(
      {
        success: false,
        error: toPublicDatabaseError(error, "Unable to delete saved search."),
      },
      { status: 500 },
    );
  }
