...
export async function PATCH(
  ...
  try {
    ...
    const existing = await prisma.guestPostOrder.findUnique({
    ...
    const updated = await prisma.guestPostOrder.update({
    ...
    return NextResponse.json(
      {
        success: false,
        error: toPublicDatabaseError(error, "Failed to update guest post order."),
      },
      { status: 500 },
    );
  }
