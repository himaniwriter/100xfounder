...
export async function generateMetadata({ params }: StagePageProps): Promise<Metadata> {
  ...
  if (!context) {
    return { title: "Stage Not Found | 100Xfounder" };
  }

  const canonical = `${getSiteBaseUrl()}/stages/${params.slug}`;
  return {
...
export default async function StagePage({ params }: StagePageProps) {
  ...
  const schema = {
    ...
    "@graph": [
      {
        "@type": "CollectionPage",
        "@id": `${baseUrl}/stages/${params.slug}#webpage`,
        url: `${baseUrl}/stages/${params.slug}`,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: context.label, item: `${baseUrl}/stages/${params.slug}` },
        ],
      },
    ],
  };
    ...
    <main className="min-h-screen bg-[#050505] text-[#EDEDED]">
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <header className="rounded-2xl border border-white/15 bg-white/[0.03] p-6 backdrop-blur-[40px]">
          <p className="text-xs uppercase tracking-[0.15em] text-zinc-500">Stage Hub</p>
